...
import { api, type TestExecution, type ExecuteTestCaseResponse } from '../../../lib/api';
...
export default function ReportViewer(): JSX.Element {
  ...
  const [lastResult, setLastResult] = useState<Record<string, ExecuteTestCaseResponse>>({});
  const [executions, setExecutions] = useState<Record<string, TestExecution[]>>({});
  const [executionsTotal, setExecutionsTotal] = useState<Record<string, number>>({});
  const [showExecutions, setShowExecutions] = useState<Record<string, boolean>>({});
  ...
  const handleExecuteTest = async (id: string) => {
    setExecutingId(id);
    try {
      const result = await api.executeTestCase(id);
      setLastResult((prev) => ({ ...prev, [id]: result }));
      // Refresh executions history for this test case
      const hist = await api.listTestExecutions(id);
      setExecutions((prev) => ({ ...prev, [id]: hist.executions }));
      setExecutionsTotal((prev) => ({ ...prev, [id]: hist.total }));
      setShowExecutions((prev) => ({ ...prev, [id]: true }));
    } catch (err) {
  ...
  const handleToggleExecutions = async (id: string) => {
    const next = !showExecutions[id];
    setShowExecutions((prev) => ({ ...prev, [id]: next }));
    if (next && !executions[id]) {
      const hist = await api.listTestExecutions(id);
      setExecutions((prev) => ({ ...prev, [id]: hist.executions }));
      setExecutionsTotal((prev) => ({ ...prev, [id]: hist.total }));
    }
          ...
          <div className="space-y-6">
            {testCases.length === 0 ? (
              ...
              testCases.map((tc) => {
                ...
                const result = lastResult[tc.id];
                const tcExecutions = executions[tc.id] ?? [];
                const tcExecTotal = executionsTotal[tc.id] ?? 0;
                  ...
                  <div key={tc.id} className="rounded-lg bg-card border border-border overflow-hidden">
                    <div className="px-6 py-4 flex items-center justify-between gap-4 border-b border-border">
                      ...
                      <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
                        ...
                        {/* Run button */}
                        <button
                          onClick={() => void handleExecuteTest(tc.id)}
                          disabled={isRunning || executingId !== null}
                          className="px-3 py-1.5 rounded-lg bg-accent text-accent-foreground text-xs font-medium hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors inline-flex items-center gap-1.5"
                          title="Run test and auto-heal if failing"
                        >
                          {isRunning ? (
                            <><svg className="w-3 h-3 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" strokeWidth="2" strokeOpacity="0.3" /><path fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Running</>
                          ) : (
                        ...
                        {/* History toggle */}
                        <button
                          onClick={() => void handleToggleExecutions(tc.id)}
                          className="px-3 py-1.5 rounded-lg bg-muted hover:bg-border text-foreground text-xs font-medium transition-colors inline-flex items-center gap-1.5"
                        >
                    ...
                    {result && (
                      <div className={`px-6 py-2 text-xs font-medium flex items-center gap-2 border-b border-border
                        ${result.status === 'passed' ? 'bg-emerald-950/20 text-emerald-300' :
                          result.status === 'healed' ? 'bg-purple-950/20 text-purple-300' :
                          'bg-red-950/20 text-red-300'}`}>
                        <span className={`w-1.5 h-1.5 rounded-full inline-block
                          ${result.status === 'passed' ? 'bg-emerald-400' :
                            result.status === 'healed' ? 'bg-purple-400' : 'bg-red-400'}`} />
                        Last run: {result.status} in {result.duration}ms
                        {result.healed && ' (auto-healed ✓)'}
                      </div>
                    )}

                    {/* Diagnosis from last run */}
                    {result?.diagnosis && (
                    ...
                    {/* Execution history */}
                    {showExecutions[tc.id] && (
                      <div className="px-6 py-4 border-t border-border bg-muted/10">
                        <HealingHistory executions={tcExecutions} total={tcExecTotal} />
                      </div>
                    )}
                  </div>
                );
